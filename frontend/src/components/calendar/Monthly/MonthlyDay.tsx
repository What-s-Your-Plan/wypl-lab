...
function MonthlyDay({ date, schedules, isCurrentMonth }: MDayProps) {
      ...
      if (idx < 3) {
        if (schedule.length > 0) {
          ...
          if (isSameDay(start, date) || date.getDay() === 0){
            const width = Math.min(7 - date.getDay(), getDateDiff(date, end) + 1)
        ...
        if (schedule.length === 0) {
          ...
          return <span className="truncate h-4 absolute top-12">+ {schedule.length}</span>;
        }
      }
    });
  };
  ...
  return (
    <S.DateContainer>
      <S.DateSpan $isCurrentMonth={isCurrentMonth} $day={date.getDay()}>
        {date.getDate()}
      </S.DateSpan>
