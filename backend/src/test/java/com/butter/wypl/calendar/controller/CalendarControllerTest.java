...
public class CalendarControllerTest extends ControllerTest {
	...
	class groupCalendar {
		...
		void getGroupCalendarNoDate() throws Exception {
			...
			// When
			ResultActions resultActions = mockMvc.perform(
				RestDocumentationRequestBuilders.get("/calendar/v1/calendars/{type}/{groupId}", CalendarType.MONTH, 1)
					.header(HttpHeaders.AUTHORIZATION, AUTHORIZATION_HEADER_VALUE)
					.contentType(MediaType.APPLICATION_JSON)
			);
		...
		resultActions.andDo(print())
			.andDo(document("calendar/year",
				...
				responseFields(
					...
					fieldWithPath("body.blocks[].time").type(JsonFieldType.NUMBER).description("해당 날짜의 총 시간")
				)
			))
			.andExpect(status().isOk());
