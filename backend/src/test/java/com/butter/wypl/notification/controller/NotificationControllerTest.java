		...
		actions.andDo(print())
				.andDo(document("notification",
						...
						responseFields(
								...
								fieldWithPath("body.notifications[].member_id").type(JsonFieldType.NUMBER)
										.description("회원ID"),
								fieldWithPath("body.notifications[].message").type(JsonFieldType.STRING)
										.description("메시지"),
								fieldWithPath("body.notifications[].is_read").type(JsonFieldType.BOOLEAN)
										.description("알림읽음 여부"),
								fieldWithPath("body.notifications[].is_acted").type(JsonFieldType.BOOLEAN)
										.description("회원 action 처리여부"),
								fieldWithPath("body.notifications[].type_code").type(JsonFieldType.STRING)
										.description("알림 타입 코드"),
								fieldWithPath("body.notifications[].target_id").type(JsonFieldType.NUMBER)
										.description("알림 대상 ID"),
								fieldWithPath("body.total_notification_count").type(JsonFieldType.NUMBER)
										.description("총 알림개수"),
								fieldWithPath("body.total_page_count").type(JsonFieldType.NUMBER)
										.description("총 페이지 수"),
