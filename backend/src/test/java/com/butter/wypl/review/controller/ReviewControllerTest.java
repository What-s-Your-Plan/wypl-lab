		...
		resultActions.andDo(print())
				.andDo(document("review/getDetailReview",
						...
						responseFields(
								...
								fieldWithPath("body.schedule.schedule_id").type(JsonFieldType.NUMBER)
										.description("일정 id"),
								fieldWithPath("body.schedule.title").type(JsonFieldType.STRING).description("일정 제목"),
								fieldWithPath("body.schedule.description").type(JsonFieldType.STRING)
										.description("일정 묘사"),
								fieldWithPath("body.schedule.start_date").type(JsonFieldType.STRING)
										.description("일정 시작 일"),
								fieldWithPath("body.schedule.end_date").type(JsonFieldType.STRING)
										.description("일정 끝 일"),
								...
								fieldWithPath("body.schedule.label.color").optional()
										.type(JsonFieldType.STRING)
										.description("라벨 색상"),
								fieldWithPath("body.schedule.member_count").type(JsonFieldType.NUMBER)
										.description("스케줄의 참여 멤버 수"),
								...
								fieldWithPath("body.schedule.members[].profile_image").type(JsonFieldType.STRING)
										.optional()
	...
	void getReviewsByMember() throws Exception {
		...
		given(reviewReadService.getReviews(anyInt(), anyInt(), any(ReviewType.class), any(LocalDate.class),
				...
				.willReturn(ReviewListResponse.from(List.of(
						...
						ReviewResponse.from(review2,
								ReviewContentsFixture.REVIEW_CONTENTS.toReviewContents().getContents()
										.get(1))
				)));
		// When
		ResultActions resultActions = mockMvc.perform(
				RestDocumentationRequestBuilders.get("/review/v1/reviews/{type}", ReviewType.OLDEST)
						...
						.queryParam("lastDate", "2024-05-11")
		);
		...
		resultActions.andDo(print())
				.andDo(document("review/getReviewsByMember",
						...
						queryParameters(
								...
								parameterWithName("lastDate").optional().description("끝 기준일"),
						...
						responseFields(
								...
								fieldWithPath("body.reviews[].review_id").type(JsonFieldType.NUMBER)
										.description("리뷰 인덱스"),
								fieldWithPath("body.reviews[].created_at").type(JsonFieldType.STRING)
										.description("리뷰 생성일"),
