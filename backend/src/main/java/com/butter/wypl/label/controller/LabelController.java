...
public class LabelController {
	...
	public ResponseEntity<Message<LabelListResponse>> getLabelsByMemberId(
		...
		return ResponseEntity
				.ok().body(
						Message.withBody("멤버 id로 라벨 리스트 조회 성공",
								labelReadService.getLabelsByMemberId(authMember.getId()))
				);
	}
	...
	public ResponseEntity<Message<LabelResponse>> updateLabel(
		...
		return ResponseEntity
				.ok().body(
						Message.withBody("라벨 수정 성공",
								labelModifyService.updateLabel(authMember.getId(), labelId, labelRequest))
				);
	}
